window.addEventListener("load", function () {
  ...
  class Projectile {
    ...
    draw(context) {
      // draw the image for the projectiles
  ...
  class Player {
    constructor(game) {
      ...
      // a powerup will last for a while and after
      // will make the player go back to the normal state
      this.powerUp = false;
      this.powerUpTime = 0;
      this.powerUpLimit = 10000;
    }
    // the deltaTime needs to be passed to the player update now
    update(deltaTime) {
      ...
      if (this.powerUp) {
        if (this.powerUpTime > this.powerUpLimit) {
          ...
          this.frameY = 1;
          // extra recharging capabilities for the ammo
          // for fast reloads
          this.game.ammo += 0.1;
        }
      }
    ...
    shootTop() {
      ...
      // call the tail laser when in powerup mode
      if (this.powerUp) this.shootBottom();
    }
    // in powerup mode we also shoot free ammo from the tail
    shootBottom() {
      if (this.game.ammo > 0) {
    ...
    enterPowerUp() {
      // reset the timer for when we collide with a lucky fish
  ...
  class Game {
    ...
    update(deltaTime) {
      ...
      // pass deltaTime to the player update method
      // to handle the powerup logic
      ...
      this.enemies.forEach((enemy) => {
        ...
        if (this.checkCollision(this.player, enemy)) {
          ...
          // if we collide with a lucky fish,
          // we go into powerup mode!
